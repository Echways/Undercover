...
def upgrade() -> None:
    ...
    sa.ForeignKeyConstraint(["category_id"], ["categories.id"], name=op.f("fk_words_category_id_categories"), ondelete="CASCADE"),
    sa.PrimaryKeyConstraint("id", name=op.f("pk_words")),
    sa.UniqueConstraint("category_id", "text", name=op.f("uq_words_category_id_text"))
    )
    op.create_index("ix_words_active_by_category", "words", ["category_id"], unique=False, postgresql_where=sa.text("is_active"))
    op.create_table("game_sessions_log",
    ...
    sa.ForeignKeyConstraint(["word_id"], ["words.id"], name=op.f("fk_game_sessions_log_word_id_words")),
    sa.PrimaryKeyConstraint("id", name=op.f("pk_game_sessions_log"))
    )
    op.create_table("spy_hints",
    sa.Column("id", sa.Integer(), nullable=False),
    sa.Column("word_id", sa.Integer(), nullable=False),
    sa.Column("hint_text", sa.Text(), nullable=False),
    sa.ForeignKeyConstraint(["word_id"], ["words.id"], name=op.f("fk_spy_hints_word_id_words"), ondelete="CASCADE"),
...
def downgrade() -> None:
    ...
    op.drop_index("ix_words_active_by_category", table_name="words", postgresql_where=sa.text("is_active"))
    op.drop_table("words")
