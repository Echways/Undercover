...
def test_opening_a_ballot_replaces_whatever_was_there() -> None:
    state = make_state(ballot=Ballot(options=["9"], votes={100: "9"}))

    open_ballot(state, ["0", "1"])

    assert state.ballot == Ballot(options=["0", "1"])


def test_a_revote_is_marked_as_one() -> None:
    state = make_state()

    open_ballot(state, ["0", "1"], revote=True)
...
def test_an_accepted_vote_is_recorded() -> None:
    ...
    assert cast(state, 100, "1") is None
    assert state.ballot is not None
...
def test_a_stranger_is_told_they_are_not_playing() -> None:
    ...
    assert cast(state, 999, "1") is Refusal.NOT_A_VOTER
...
def test_an_eliminated_player_is_told_they_are_out() -> None:
    ...
    assert cast(state, 102, "1") is Refusal.IS_OUT
...
def test_nobody_votes_twice() -> None:
    ...
    assert cast(state, 100, "1") is Refusal.ALREADY_VOTED
    assert state.ballot is not None
...
def test_a_button_from_a_stale_screen_is_rejected() -> None:
    ...
    assert cast(state, 100, "3") is Refusal.UNKNOWN_OPTION


def test_voting_without_a_ballot_is_rejected_too() -> None:
    assert cast(make_state(), 100, "0") is Refusal.UNKNOWN_OPTION


def test_the_tally_counts_every_option_including_the_unloved() -> None:
    state = make_state()
    open_ballot(state, ["0", "1", "2"])
    cast(state, 100, "0")
    cast(state, 101, "0")
    cast(state, 102, "1")
...
def test_the_hot_seat_host_alone_makes_a_full_turnout() -> None:
    ...
    cast(state, HOST_ID, Direction.VOTE)

    assert turnout(state) == (1, 1)


def vote_all(state: GameSessionState, choices: dict[int, str]) -> None:
    for voter_id, option in choices.items():
        assert cast(state, voter_id, option) is None


def test_the_direction_is_undecided_while_the_table_is_still_tapping() -> None:
    state = make_state()
    open_ballot(state, [Direction.ROUND, Direction.VOTE])
    cast(state, 100, Direction.VOTE)
...
def test_the_direction_is_settled_by_half_the_table_plus_one() -> None:
    state = make_state()
    open_ballot(state, [Direction.ROUND, Direction.VOTE])
    vote_all(state, {100: Direction.VOTE, 101: Direction.VOTE, 102: Direction.VOTE})
...
def test_the_elimination_stays_silent_until_everyone_has_voted() -> None:
    state = make_state()
    open_ballot(state, ["0", "1", "2", "3"])
    vote_all(state, {100: "1", 101: "1", 102: "1"})
...
def test_a_clear_majority_sends_one_player_out() -> None:
    state = make_state()
    open_ballot(state, ["0", "1", "2", "3"])
    vote_all(state, {100: "1", 101: "1", 102: "1", 103: "0"})
...
def test_a_tie_sends_only_the_leaders_to_a_second_round() -> None:
    state = make_state()
    open_ballot(state, ["0", "1", "2", "3"])
    vote_all(state, {100: "1", 101: "1", 102: "0", 103: "0"})
